...
export const generateUpdateArgs = (generatorConfig: GeneratorConfig, model: DMMF.Model): Resource => {
  const args = generatorConfig.getServiceArgs(model, 'Update');
  const argsFields = model.fields.filter((field) => !field.relationName && !field.isUpdatedAt);
